...
const readNotificationAction = (notificationId) => async (dispatch) => {
  try {
    ...
    const response = await customAxios.put(`/notification/read/${notificationId}`);
    ...
    dispatch(readNotificationFailure(error?.response?.data?.message || "Error while reading notification"));
  }
};
