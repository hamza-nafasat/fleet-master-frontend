...
const getSingleTruckAction = (truckId) => async (dispatch) => {
  ...
  try {
    ...
    // console.log(error);
    dispatch(
      getSingleTruckFailure(error?.response?.data?.message || "Error ocurred while getting single truck")
    );
  }
};
...
const getAllTrucksAction = () => async (dispatch) => {
  ...
  try {
    ...
    // console.log(error);
    dispatch(getAllTrucksFailure(error?.response?.data?.message || "Error ocurred while getting all trucks"));
  }
};
...
const attachDeviceToTruckAction = (truckId, deviceId) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      attachDeviceToTruckFailure(error?.response?.data?.message || "Error ocurred while attaching device")
    );
  }
};
...
const detachDeviceFromTruckAction = (truckId, deviceId) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      detachDeviceFromTruckFailure(error?.response?.data?.message || "Error ocurred while detaching device")
    );
  }
};
export {
